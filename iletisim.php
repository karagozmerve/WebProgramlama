...
?>
	...
	<form action=" " method="post">
		...
		<h1 style="color: #680001;"><b>Bana Mail Gönderin :)</b></h1>
		<h3><i>(*) işaretli yerleri lütfen doldurun...</i></h3>
		<hr>
         <h3>Ad Soyad(*)</h3>
        <input type="text" name="name" value="<?php echo $sonuc['ad']; ?>" required width="40%">
		<h3>Doğum Tarihi(*)</h3>
		<input type="date" name="tarih" value="<?php echo $sonuc['dogumtarihi']; ?>"required>
		<h3>Telefon Numarası(*)</h3>
		<input type="tel" name="telefon" value="<?php echo $sonuc['telnum']; ?>"required>
		<h3>E-Posta Adresi(*)</h3>
        <input type="email" name="e-mail" value="<?php echo $sonuc['mail']; ?>"required>           
        <h3>İleti Konusu(*)</h3>
        <input type="text" name="konu" required>
		<h3>İleti(*)</h3>
		<textarea rows="20" cols="80" name="ileti" required > </textarea>
    	<input type="hidden" name="_next" value="https://site.io/iletisim.html"> <br /><br />
    	<input type="submit" class="gonder" name="gonder" value="MAİL GÖNDER"><br />
...
$ileti=$_POST['ileti'];
	...
	$sqlekle="INSERT INTO mail(gonderen,konu,mail) VALUES ('$mail','$konu','$ileti')";
